...
- **`jedissource/`** — OSGi component, factory PID `rahla.jedissource`. Each `rahla.jedissource-<name>.cfg` becomes a `rahla.jedissource.JedisSource` service (pooled Jedis), filterable by `jedisSourceName`. Note: source path is `src/main/java/rahla.jedissource/...` (literal dotted directory name, not a `rahla/jedissource/` tree).
...
- **`loki-appender/`** — Log4j2 appender for Grafana Loki, repackaged as a **`Fragment-Host: org.ops4j.pax.logging.pax-logging-log4j2`** bundle so it loads inside the pax-logging classloader. The pax-logging bundles come from the `rahla-logging` feature (see below), not Karaf's defaults.
