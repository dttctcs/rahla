...
Releases use `maven-release-plugin` (the recent commits `[maven-release-plugin] prepare release/next iteration` show the convention); tags are then picked up by `.forgejo/workflows/build.yaml` (Forgejo Actions — GitHub ignores `.forgejo/`) to publish `docker.io/datatactics/rahla`. The version bump itself runs either locally (`mvn release:prepare`) or from the Forgejo UI via `.forgejo/workflows/release.yaml` — a `workflow_dispatch` workflow (inputs: release version, next dev version, dry run) that does nothing but `release:prepare`. It needs the secret **`RELEASE_TOKEN`** (a PAT with `write:repository`, optionally paired with the variable `RELEASE_USER`): Forgejo never triggers a workflow from a push authored with the built-in token, so a tag pushed with that token would land but leave `build.yaml` unrun. `-Dtag=v${version}` is passed explicitly because the pom sets no `<tagNameFormat>` and the plugin default would produce `reactor-X.Y.Z`. Only `release:prepare` runs, never `release:perform` — there is no `<distributionManagement>`. Two release objects are created from the same `CHANGELOG.md` section, by two separate workflows — **Forgejo never runs anything under `.github/`**, because it ignores that directory entirely as long as `.forgejo/` exists (so the `github.server_url` guard in `release.yaml` is belt-and-braces, not the mechanism):

- **Forgejo** (`repo.datatactics.dev`): job `forgejo-release` in `.forgejo/workflows/build.yaml`, tag-gated, `POST /api/v1/repos/${{ github.repository }}/releases` with `${{ github.token }}` (HTTP 409 = release already exists, treated as success so re-runs are safe).
- **GitHub mirror** (<https://github.com/dttctcs/rahla>): `.github/workflows/release.yaml` — no build runs there; the release exists so Renovate consumers can track rahla via the `github-releases` datasource.

Keep the two changelog-extraction `awk` snippets in sync when you touch either. The Maven artifact repo `https://mvn.datatactics.dev` (id `dtacs`) is required for some non-public Siddhi artifacts (`siddhi-store-rdbms` `7.0.16-dtacs`, `siddhi` `5.1.24-dt`).
...
- **`features/<name>/`** — Karaf feature XML modules (`<packaging>feature</packaging>`). The `features/rahla` feature used to pin `io.undertow/undertow-{core,servlet}` while pax-web lagged behind; the pin was dropped in 1.3.5 (pax-web `8.0.35` ships `2.2.39.Final` itself). Don't reintroduce a pin without also blacklisting pax-web's own version in `assembly/pom.xml` — otherwise both land in the assembly. The `features/logging` feature is `rahla-logging` and replaces Karaf's stock pax-logging bundles.
