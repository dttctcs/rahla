...
Releases use `maven-release-plugin` (the recent commits `[maven-release-plugin] prepare release/next iteration` show the convention); tags are then picked up by `.forgejo/workflows/build.yaml` (Forgejo Actions — GitHub ignores `.forgejo/`) to publish `docker.io/datatactics/rahla`. Two release objects are created from the same `CHANGELOG.md` section, by two separate workflows — **Forgejo never runs anything under `.github/`**, because it ignores that directory entirely as long as `.forgejo/` exists (so the `github.server_url` guard in `release.yaml` is belt-and-braces, not the mechanism):
